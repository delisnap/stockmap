...
`data.js`의 관계 56건은 **출처 대조 없이 작성**된 것이다. 2026-07-19에 그중 14건을 웹 검색으로 검증해
4건 수정 · 1건 추가 · 1건 삭제했다 (상세: `data.js` 상단 주석).

**나머지 42건은 여전히 미검증**이다. "확인됨"이 아니라 "확인하지 않음"이라는 뜻이고,
특히 개별 계약을 주장하는 항목(전력·냉각 납품 등)은 근거가 약하다.

검증에서 드러난 패턴: 구조적인 관계(누가 누구에게 칩을 만들어주는가)는 잘 버텼고,
**계약·투자·독점 조건처럼 시점을 타는 관계가 무너졌다.** 틀린 3건 모두 최근 6개월 안에 바뀐 것이었다.
